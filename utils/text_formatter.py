...
【キャラクターシナリオのルール】
1. **冒頭（最初の1ページ分）はテロップ（地の文）のみ**にしてください。【キャラ名】タグは付けないでください
2. テロップで視聴者の注意を引いた後、キャラクターの会話に入ってください
3. キャラクターのセリフは「【キャラ名】セリフ」の形式で書いてください
4. キャラクター名タグ（【〇〇】）は行の先頭に付け、セリフ文字数には含めません
5. 各キャラクターのプロフィール（年代・性別・見た目・雰囲気・背景・口調）を忠実に反映してください
6. 特に「口調」の設定は最重要です。キャラごとに話し方を明確に区別してください
7. 会話や掛け合いを自然に組み立ててください
8. ナレーション（キャラ名なしの地の文）もシナリオ途中に適宜入れてOKです
9. 使用キャラクター: {char_names}
...
4. **冒頭（最初の数行）はテロップ（地の文）のみ**にしてください。【キャラ名】タグは絶対に付けないでください。視聴者の注意を引くフレーズを入れてください
5. テロップの後からキャラクターの会話・掛け合いに入ってください
6. シナリオ終盤には、誘導文テンプレートを参考に給付金制度の紹介や行動喚起を自然に組み込んでください
7. 最後の締めは「詳しく知りたい方は〜」「気になる方は〜」のように、次のアクションにつなげる文で終わってください
...
3. TikTok動画として視聴者を引き込む構成にしてください
4. **各パターンの冒頭（最初の数行）はテロップ（地の文）のみ**にしてください。【キャラ名】タグは絶対に付けないでください。視聴者の注意を引くフレーズを入れてください
5. テロップの後からキャラクターの会話・掛け合いに入ってください
6. 各パターンの終盤には、誘導文テンプレートを参考に給付金制度の紹介や行動喚起を自然に組み込んでください
7. 最後の締めは「詳しく知りたい方は〜」「気になる方は〜」のように、次のアクションにつなげる文で終わってください
